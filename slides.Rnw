...
\begin{itemize}
\item A parsimonious, computational scalable model of gene functional evolution
\item An extension to a well studied family of statistical models for social networks
\end{itemize}

\large {\bf Accomplished so far}
\begin{itemize}
\item Two new statistical models for complex (no-IID) systems.
\item 5 journal publications: 2 on the (JOSS, Stata Journal, Journal of Behavioral)
\item x packages/libraries built (2 on CRAN now) ergmito, fmcmc, slurmR, aphylo, polygons, pruner, netplot, phyloXML,  
\item a
\end{itemize}
