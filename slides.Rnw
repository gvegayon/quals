...
\caption{Random sample of 5 networks simulated using a negative}
\end{figure}
\end{minipage}
\hfill
\begin{minipage}{.55\linewidth}
...
\uncover<5->{
<<random-tree, echo=FALSE, messages=FALSE, out.width=".75\\linewidth", fig.align='center', fig.cap="Random annotated phylogenetic tree.">>=
...
\end{minipage}
\vfill
\uncover<6->{\textcolor{usccardinal}{\bf We can use the evolutionary tree to infer presence/absence of gene functions!}}
...
\begin{itemize}
	...
	\item<4-> \textcolor{leafnode}{There's a chance of error.}
\end{itemize}
\end{minipage}
%
...
\frametitle{An evolutionary model of gene functions (cont'd)}

\includegraphics[width=.5\linewidth]{aphylo.pdf}

\begin{itemize}[<+->]
\item Gain and loss also depend on the type of the parent node
\hyperlink{duplicationvsspeciation}{\beamerbutton{speciation vs duplication}}

\item We use Felsensteins' pruning algorithm \hyperlink{aphyloalgorithmicview}{\beamergotobutton{algorithmic view}} (linear complexity)

\item Full framework has been implemented in the \aphylopkg{} R package \hyperlink{aphylopkg}{\beamergotobutton{more}}.
\end{itemize}
...
\centering
\scalebox{0.6}{
\begin{tabular}{l*{2}{m{0.14\linewidth}}*{2}{>{\bf\large}m{0.14\linewidth}}*{4}{m{0.14\linewidth}}}
  \toprule
 & (1) & (2) & (3) & (4) & (5) & (6) & (7) & (8) \\ 
  \midrule
