...
plot(aphylo::rdrop_annotations(tree, .2),
                   main = "")
par(oldpar)
@
}
...
\begin{tabular}{ll*{2}{m{0.3\linewidth}<\centering}}
 ...
 & & (1) & (2)  \\ 
  \midrule
  \multicolumn{4}{l}{Mislabeling probabilities} \\
  & $\psi_0$ & 0.23 & 0.25 \\ 
  & $\psi_1$ & 0.01 & 0.01 \\ 
  \multicolumn{4}{l}{Gain/Loss at duplication nodes} \\
  & $\mu_{d0}$ & 0.97 & 0.96 \\ 
  & $\mu_{d1}$ & 0.52 & 0.58 \\ 
  \multicolumn{4}{l}{Gain/Loss at speciation nodes} \\
  & $\mu_{s0}$ & 0.05 & 0.06 \\ 
  & $\mu_{s1}$ & 0.01 & 0.02 \\ 
  \multicolumn{4}{l}{Root node} \\
  & $\pi$ & 0.81 & 0.45  \\ 
\midrule   
  & Prior & Uniform & Beta  \\ 
  & AUC & 0.69 & 0.67 \\
